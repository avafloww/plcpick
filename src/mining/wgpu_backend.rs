    ...
    fn run(
        ...
        let (device, queue) = pollster::block_on(adapter.request_device(
            &wgpu::DeviceDescriptor {
                ...
                // SAFETY: we use EXPERIMENTAL_PASSTHROUGH_SHADERS to bypass naga's
                // pathologically slow SPIR-V backend on large shaders. Our SPIR-V is
                // generated by naga itself, so it's well-formed.
        ...
        // 4. Compose shaders and create modules
        eprintln!("[wgpu] compiling shaders...");
        let ec_shader_src = compose_ec_pass_shader();
        let hash_shader_src = compose_hash_pass_shader();

        // Compile WGSL→SPIR-V ourselves with naga (17ms) and load via passthrough API.
        // This is critical: wgpu always defers naga SPIR-V codegen to pipeline creation
        // (SEPARATE_ENTRY_POINTS workaround is unconditionally enabled), where bounds
        // checking makes it pathologically slow on 2500+ line shaders. The passthrough
        // API sends SPIR-V directly to the Vulkan driver, bypassing naga entirely.
        let ec_spv = compile_wgsl_to_spirv(&ec_shader_src, "ec_pass")?;
        let hash_spv = compile_wgsl_to_spirv(&hash_shader_src, "hash_pass")?;
        eprintln!("[wgpu] SPIR-V compiled ({} + {} words)", ec_spv.len(), hash_spv.len());

        // SAFETY: SPIR-V was just generated by naga from our validated WGSL
        eprintln!("[wgpu] creating shader modules via passthrough...");
        let t = Instant::now();
        let ec_module = unsafe {
            device.create_shader_module_passthrough(wgpu::ShaderModuleDescriptorPassthrough {
                label: Some("ec_pass"),
                spirv: Some(std::borrow::Cow::Borrowed(&ec_spv)),
                ..Default::default()
            })
        };
        eprintln!("[wgpu] ec_pass module created in {:?}", t.elapsed());
        let t = Instant::now();
        let hash_module = unsafe {
            device.create_shader_module_passthrough(wgpu::ShaderModuleDescriptorPassthrough {
                label: Some("hash_pass"),
                spirv: Some(std::borrow::Cow::Borrowed(&hash_spv)),
                ..Default::default()
            })
        };
        eprintln!("[wgpu] hash_pass module created in {:?}", t.elapsed());

        // 5. Create explicit bind group layouts (required for passthrough shaders —
        // wgpu can't introspect raw SPIR-V for auto-layout)
        ...
        // ec_pass: binding 0=rw(scalars), 1=ro(g_table_x), 2=ro(g_table_y), 3=ro(stride_g),
        //          4=ro(unsigned_template), 5=ro(unsigned_offsets), 6=rw(results), 7=uniform(params)
        let ec_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("ec_bind_group_layout"),
            entries: &[
                storage_rw(0), storage_ro(1), storage_ro(2), storage_ro(3),
                storage_ro(4), storage_ro(5), storage_rw(6), uniform(7),
            ],
        });
        let ec_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ec_pipeline_layout"),
            bind_group_layouts: &[&ec_bind_group_layout],
            immediate_size: 0,
        });

        // hash_pass: binding 0=ro(ec_results), 1=ro(signed_template), 2=ro(pattern),
        //            3=rw(matches), 4=rw(match_count), 5=uniform(params)
        let hash_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("hash_bind_group_layout"),
            entries: &[
                storage_ro(0), storage_ro(1), storage_ro(2),
                storage_rw(3), storage_rw(4), uniform(5),
            ],
        });
        ...
        // 6. Create compute pipelines with explicit layouts
        let compilation_options = wgpu::PipelineCompilationOptions {
            zero_initialize_workgroup_memory: false,
            ..Default::default()
        };
        eprintln!("[wgpu] creating pipelines...");
        let t = Instant::now();
        let ec_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("ec_pass_pipeline"),
            layout: Some(&ec_pipeline_layout),
            module: &ec_module,
            entry_point: Some("main"),
            compilation_options: compilation_options.clone(),
            cache: None,
        });
        eprintln!("[wgpu] ec_pass pipeline created in {:?}", t.elapsed());
        ...
        eprintln!("[wgpu] hash_pass pipeline created in {:?}", t.elapsed());
        ...
        eprintln!("[wgpu] setup complete, entering mining loop...");
        // 9. Main mining loop
...
// scalar_mul_g_windowed — same as ec_pass.wgsl
